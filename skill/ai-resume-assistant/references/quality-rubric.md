...
This rubric scores the resume document, not the person's total market value. Report formal title, relevant tenure, employer credibility, product scale, commercial results, public reputation, application channel, and hard JD fit in a separate candidate-signal assessment. A strong candidate can have an overloaded resume; a strong resume cannot erase a hard experience mismatch.
...
For sample resumes and “perfect resume” articles, state the evidence boundary before scoring. If the artifact is a teaching template, marketing persona, tutorial project, or public expert's illustrative resume, score the text as presented but do not treat it as the market baseline for ordinary applicants. Give a conditional upper score only when missing metric definitions or ownership evidence would materially change the result.
...
- Final text that cannot be traced to verified or user-confirmed evidence.
...
- A number in nearly every bullet despite missing baselines, samples, measurement windows, or quality guardrails.
- Percent of AI-generated code presented as productivity or quality without human-owned design, verification, rework, and operating evidence.
- Public reputation or a strong employer used to excuse weak attribution, unsupported metrics, or an unclear project story.
- V1 / V2 / V3 used as the project spine even though the observable user problem and product behavior are unchanged or absent.
- Internal safety levels shown without defining dangerous inputs, blocked flows, escalation behavior, or user-facing response.
- Exposure volume used as the headline when smaller but stronger repeat-behavior or user-outcome evidence exists.
...
9. Verify evidence provenance for every final line before PDF generation.
10. Verify the reconstructed framework was deliberately chosen for the target and no legacy structure was inherited by default.
11. Read representative lines aloud; return wording the user would not naturally explain to the coaching loop.
12. Remove framework names temporarily; if the product, user task, and result disappear, fail the product-clarity gate and reconstruct the section.

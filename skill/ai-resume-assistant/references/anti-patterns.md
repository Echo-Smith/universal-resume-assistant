...
Repair: ask the user to confirm the original stack and, when applicable, mark a later refactor as a separately dated event. In co-author mode, draft the corrected chronology only after the dates and stack are confirmed.
...
When a sample resume comes from the author of the tutorial, a course vendor, a resume builder, or an influencer article, classify it as a reference or marketing artifact before using it as a candidate benchmark. Its structure may be useful even when the persona, metrics, or project ownership are illustrative.
...
### Quantification theater

Signal: nearly every bullet contains a percentage, latency reduction, accuracy gain, or cost saving, but the resume does not define comparable baselines, samples, workload, measurement window, attribution, or a preserved quality condition.

Do not require every bullet to contain a number. Do not recommend a quick benchmark merely to manufacture a result. A useful metric tests the claim that matters to the user or business; a decorative percentage increases interview risk.

Repair in this order:

1. add the metric definition, scope, baseline, and method when they exist;
2. report final attainment with conditions when there is no fair baseline;
3. use adoption, shipped behavior, eliminated failure mode, rollout scope, or external proof when they better represent the result;
4. remove the number when it cannot be defended.

### AI-generated-code theater

The proportion of code generated by AI is an input or process description, not automatic evidence of productivity, ownership, or quality. Ask who defined requirements and architecture, how generated code was reviewed and tested, how much rework occurred, and what operating result improved. Keep the percentage only when its measurement and relevance are defensible; otherwise describe the human-owned workflow and verification controls.
...
This keeps the old resume's strategic mistakes and encourages line-by-line polishing. Treat the old document as a fact source and atomize its evidence. In audit mode, help the user choose the new framework; in co-author mode, propose a target-led framework and explain its tradeoffs before drafting. Read [reconstruction-and-voice.md](reconstruction-and-voice.md).
...
Do not call this proof of AI authorship. Return it as a voice and distinctiveness problem. Restore project-specific details from confirmed evidence. Ask the user how they would explain any phrase that remains unnatural or difficult to defend.
...
6. In audit mode, revise locally. In co-author mode, draft from confirmed atoms. In both cases, ensure the statement is weaker than or equal to the available evidence, never stronger.

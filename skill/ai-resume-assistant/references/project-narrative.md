...
Use this guide when the reconstructed resume describes Agent, RAG, platform, framework, or other architecture-heavy work. Extract project facts without preserving a legacy title, bullet order, or category. Examples illustrate reasoning patterns and must never be copied as candidate claims. In co-author mode, draft a new framing from confirmed evidence; in audit mode, use the guide to formulate questions.
...
For product and AI-product roles, also run [product-clarity-gate.md](product-clarity-gate.md). The project must remain understandable after framework names are removed.
...
In audit mode, ask the user to choose or write the framing. In co-author mode, reconstruct the prose from confirmed evidence and ask the user to verify any new interpretation.
...
- Are V1 / V2 labels subordinate to the user problem and decision rather than used as the story itself?
- When safety is relevant, are risky inputs and the exact system response defined before internal risk codes?
